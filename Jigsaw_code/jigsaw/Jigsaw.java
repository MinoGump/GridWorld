...
public class Jigsaw {
	...
	public boolean BFSearch() throws IOException {
		...
		// (1)将起始节点放入openList中
		openList.addElement(this.beginJNode);
		...
		while (this.openList.isEmpty() != true && searchedNodesNum <= maxNodesNum) {
			...
			while (!followJNodes.isEmpty()) {
				openList.addElement(followJNodes.elementAt(0));
	...
	private void estimateValue(JigsawNode jNode) {
		...
		// 2. f2(n) = the distance of endJNode and jNode
		int s2 = 0;
		for(int index =1 ; index <= dimension * dimension; index++){
			if (index !=  jNode.getNodesState()[0]) {
				int endRow = ( index - 1 ) / dimension;
				int endCol = ( index - 1 ) % dimension;
				int jRow = (jNode.getNodesState()[index] - 1) / dimension;
				int jCol = (jNode.getNodesState()[index] - 1) % dimension;
				/*
				if (Math.abs(endRow - jRow) > Math.abs(endCol - jCol)) {
					s2 += 10 * Math.abs(endRow - jRow) + 6 * Math.abs(endCol - jCol) ;
				} else {
					s2 += 6 * Math.abs(endRow - jRow) + 10 * Math.abs(endCol - jCol) ;
				}
				*/
				s2 += Math.abs(endRow - jRow) + Math.abs(endCol - jCol) ;
			}
		}
		//s2 = Math.abs(beginRow - jRow) + Math.abs(beginCol - jCol) ;
		// the relation of s1 and s2.
		//System.out.println("the rest : " + Integer.toString(s1*s1 - s2*s2));

		jNode.setEstimatedValue(5 * s1 + 8 * s2);
	}
